...
def save_checkpoint(state, is_best, checkpoint='checkpoint', filename='checkpoint_vgg16_H2L_freeze_cifar100.pth.tar'):
...
model.train()
torch.save(model.state_dict(), 'FreezeOut_H2L_vgg_20_cifar100_new_epochs')
...
# load the weight
net.load_state_dict(torch.load('FreezeOut_H2L_vgg_20_cifar100_new_epochs'))
...
prob = torch.from_numpy(np.array([0.9,0.8,0.8,0.7,0.7,0.6,0.6,0.5,0.5,0.4,0.4,0.3,0.3,0.2,0.2,0.1]))
