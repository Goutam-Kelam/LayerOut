...
def save_checkpoint(state, is_best, checkpoint='checkpoint', filename='checkpoint_res110_freeze_NRE.pth.tar'):
...
torch.save(model.state_dict(), 'res110_freeze_20_epochsNRE')
...
net.load_state_dict(torch.load('res110_freeze_20_epochsNRE'))

K = 110 # no.of layers (in paper no. of residual blocks)
...
count = 2
f_t_time = []
for epoch in range(start_epoch+20,args.epochs):
    count = adjust_learning_rate(optimizer, epoch,count)
...
print("____"*50)
print("LayerWise count: {}".format(f_counter))
print("\nTotal No of layers frozen: {}".format(sum(f_counter))) 
